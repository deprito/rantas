...
import { Users, Settings, Shield, Mail, Inbox, Ban, Database, ShieldAlert, DatabaseZap } from 'lucide-react';
...
import { RoleManagementContent } from './roles/page';
import { DataManagementContent } from './data-management/page';
...
function AdminLayout() {
  ...
  const canViewRoles = hasPermission(Permission.ROLE_VIEW);
  ...
  const canViewDataManagement = hasPermission(Permission.STATS_VIEW) || hasPermission(Permission.CASE_DELETE);
  const isAdmin = hasPermission('*');

  // If user can't access anything, show access denied
  if (!canViewUsers && !canViewRoles && !canViewConfig && !canViewEmailTemplates && !canViewSubmissions && !canViewBlacklist && !canViewDataManagement) {
    return (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900">
          ...
          <div className="hidden md:block">
            ...
            <div className="flex gap-2 mt-4 flex-wrap">
              ...
              {canViewDataManagement && (
                <Link href="/admin?tab=data-management">
                  <Button
                    variant={tab === 'data-management' ? 'default' : 'ghost'}
                    size="sm"
                    className="gap-2"
                  >
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {tab === 'roles' && canViewRoles && <RoleManagementContent />}
        ...
        {tab === 'data-management' && canViewDataManagement && <DataManagementContent />}
      </main>
    </div>
  );
}
